...
* @Last Modified time: 2019-03-24 11:18:24
*----------------------------------------------------------------------------*/
// File: lattice.cpp
...
void Lattice::construct(const lattice_id& id, const lattice_size& size)
{
	...
	switch (id_) {
		case lattice_id::SQUARE: 
			...
			break;
		case lattice_id::CHAIN: 
			lattice_dim_ = 1;
                        bc_ = {bc_t::PERIODIC, bc_t::OPEN, bc_t::OPEN};
			construct_chain(size);
			break;			
		default: 
...
void Lattice::construct_chain(const lattice_size& size)
{
	num_basis_sites_ = 1;
	num_sites_ = num_basis_sites_ * size_.L1() * size_.L2();
	a1_ = Vector3d(1,0,0);
