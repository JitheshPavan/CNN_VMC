...
int FockBasis::from_which_site(void) const
{
  if (proposed_move_==move_t::upspin_hop){
    return up_fr_state_;
  }
  else if(proposed_move_==move_t::dnspin_hop){
    return dn_fr_state_-num_sites_;
  }
  else{
    throw std::logic_error("FockBasis::which_site: no existing move");
  }
...
std::pair<int,int> FockBasis::exchange_move_dnpart(void) const
{
  if (proposed_move_==move_t::exchange) {
    int dn_to_site = dn_to_state_-num_sites_;
    return std::make_pair(mv_dnspin_,dn_to_site);
  }
}

std::pair<int,int> FockBasis::exchange_move_uppart(void) const
{
  if (proposed_move_==move_t::exchange) return std::make_pair(mv_upspin_,up_to_state_);
}

std::pair<int,int> FockBasis::exchange_move_frsite(void) const
{
  if (proposed_move_==move_t::exchange){
    int dn_fr_site = dn_fr_state_-num_sites_;
    int up_fr_site = up_fr_state_;
    return std::make_pair(up_fr_site,dn_fr_site);
  }
}
