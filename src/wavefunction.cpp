...
void Wavefunction::init(const wf_id& id, const Lattice& lattice, const double& hole_doping)
{
  ...
  psi_grad.resize(num_sites_,num_sites_);
}
    ...
    const int& start_pos, const bool& psi_gradient)
{
  if (lattice.id() == lattice_id::SQUARE) {
    ...
    RealVector phi_k_gradient(lattice.num_kpoints());
...
void Wavefunction::get_grad_amplitudes(ComplexMatrix& ampl_mat, const std::vector<int>& row, 
  const std::vector<int>& col) const
{
  for (int i=0; i<row.size(); ++i) {
    for (int j=0; j<col.size(); ++j) {
      ampl_mat(i,j) = psi_grad(row[i],col[j]);
    }
  }
}
